...
const config            = require(`${__dirname}/Custom/UI/config.json`);
...
const LESS_PATH         = `${__dirname}${config.lang.less.src}`;
...
gulp.task('less', function() {
    return gulp.src(LESS_PATH + '/**/*.less')
        ...
        .pipe(autoPrefixer({
            browsers: config.lang.less.browsers
        }))
        .pipe(combineMq({
            beautify: false
        }))
...
const MJML_PATH             = `${__dirname}${config.lang.mjml.src}`;
